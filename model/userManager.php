<?php class userManager extends manager{
// add One student for Secretary
 public function addStudent(Student $student){
   $query = $this->getDb()->prepare('INSERT INTO user (first_name, last_name, mail, phone, status)VALUES(:first_name, :last_name, :mail, :phone, :status )');
  ...
  // function to have a user with skill informations
  function getUsersSkill(){
  $query = $this->getDb()->query('SELECT u.id_user, u.last_name, u.first_name, s.result, s.level, s.user_id FROM user AS u INNER JOIN session AS s ON u.id_user = s.user_id');
